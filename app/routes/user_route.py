...
async def create_user(user_create: UserCreate, db: AsyncSession = Depends(get_db)) -> UserResponse:
    db_user = await UserRepository(db).create_user(**user_create.model_dump(exclude_unset=True))
    return db_user
...
async def update_user(user_id: uuid.UUID, user_update: UserUpdate, db: AsyncSession = Depends(get_db)) -> UserResponse:
    ...
    updated_user = await UserRepository(db).update_user_by_id(user_id=user_id,
...
async def register(user_create: UserCreate, db: AsyncSession = Depends(get_db)) -> dict:
    return await signup(db, **user_create.model_dump(exclude_unset=True))
